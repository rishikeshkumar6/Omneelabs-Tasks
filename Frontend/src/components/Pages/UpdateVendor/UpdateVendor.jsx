...
import updatevendorStyles from './UpdateVendor.module.css'
...
const UpdateVendor = () => {
  ...
  const Error=useSelector((state)=>state.Error)
    ...
    const handleSubmit=async (event)=>{
        ...
        if(!Input.email || !Input.mobilenumber){
          //  setErrorMessage("Please fill all input filed")
  ...
  return (
    <div className={updatevendorStyles.parent}>
     ...
     <InputControlComponents
      ...
      {
        popup && <div className={updatevendorStyles.popup}>
          <span>Profile Update Successfully</span>
          <span>Go to Home <button onClick={()=>navigate('/vendor')}>home</button>{' '}<button onClick={handlePopup}>Cancel</button></span>
        </div>
      }
    </div>
  )
